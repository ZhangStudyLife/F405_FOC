/**
  ...
  *
  * ---------------------------------------------------------------------------
  * 发送会不会阻塞 CPU？—— 上板实测（Release，168 MHz，DWT 周期计数）
  * ---------------------------------------------------------------------------
  *   HAL_CAN_AddTxMessage() 本身**不等待**：读一次 TSR，有空邮箱就填
  *   TIR/TDTR/TDHR/TDLR 并置 TXRQ 后立刻返回；三个邮箱全满则直接返回错误。
  *   源码里没有任何 while（见 stm32f4xx_hal_can.c:1252）。
  *
  *   所以"阻塞"来自调用方做什么，而不是 HAL：
  *
  *     a) 只调一次 bsp_can_send() / AddTxMessage ..........  129 周期 = 0.77 µs
  *     b) 再轮询等该邮箱 TXOK 置位（即"等发送完成"） ......20615 周期 = 122.7 µs
  *
  *   (b) 是 (a) 的 160 倍。放到 20 kHz 电流环（周期 50 µs）里看：
  *   (a) 占 1.5%，勉强可接受；(b) 占 245%，绝对不能做。
  *   （122.7 µs 略高于理论最小帧长 111 bit：数据字段全 0 触发了位填充。）
  *
  *   还有一条硬约束：**F4 的 bxCAN 只有 3 个发送邮箱**。实测在 NORMAL 模式、
  *   总线上没有第二个节点时连发 4 帧，前 3 帧进入邮箱（无 ACK 会被自动重传
  *   一直占着），第 4 帧返回错误。bsp_can_send() 此时返回 BSP_CAN_ERR_TX_FULL，
  *   **不阻塞、不等待**，由调用方决定丢弃还是排队稍后重试。
  *
  *   如果确实需要确认"发出去了没有"，正确做法是开 TX 中断、在
  *   HAL_CAN_TxMailbox0CompleteCallback 里推进状态机，而不是在调用点轮询。
  *
  * ---------------------------------------------------------------------------
